...
do
	...
	if [[ "$gossipPort" != 5004 ]] ; then
        ...
        echo "Failed for i = $i in ${outputFiles[$i]}: $msgLine1"
        	failed="T"
    	fi
		if !(grep -Eq "$msgLine2" "${outputFiles[$i]}") ; then
        echo "Failed for i = $i in ${outputFiles[$i]}: $msgLine2"
        	failed="T"
    	fi
	fi
	...
	if [[ "$gossipPort" != 5001 ]] ; then
        ...
        echo "Failed for i = $i in ${outputFiles[$i]}: $msgLine3"
        	failed="T"
    	fi
		if !(grep -Eq "$msgLine4" "${outputFiles[$i]}") ; then
        echo "Failed for i = $i in ${outputFiles[$i]}: $msgLine4"
        	failed="T"
    	fi
	fi
	...
	if [[ "$gossipPort" != 5006 ]] ; then
        ...
        echo "Failed for i = $i in ${outputFiles[$i]}: $msgLine5"
        	failed="T"
    	fi
	fi
	gossipPort=$(($gossipPort+1))
